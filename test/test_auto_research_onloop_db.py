...
class TestBriefTransactionality:
    """brief.md must always reflect a committed sub_questions row: a launch
    snapshot racing an Add Question append may not overwrite the brief with a
    row that misses the persisted question."""

    def test_launch_brief_write_is_inside_the_row_transaction(self):
        """Structural pin: the launch path's row read and its _write_brief call
        live in the SAME sync closure under BEGIN IMMEDIATE — two separate
        thread hops here reintroduce the stale-snapshot overwrite.

        Only ``execute()`` call ARGUMENTS count as evidence — a docstring
        mentioning BEGIN IMMEDIATE must not satisfy this test."""
        import textwrap

        src = textwrap.dedent(inspect.getsource(h._launch_loop))
        tree = ast.parse(src)

        def _executed_sql(fn: ast.FunctionDef) -> set[str]:
            out: set[str] = set()
            for n in ast.walk(fn):
                if (
                    isinstance(n, ast.Call)
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                ...
                if "_write_brief" in calls:
                    sql = _executed_sql(node)
                    assert any("BEGIN IMMEDIATE" in s for s in sql), (
                        "launch brief write is not inside a BEGIN IMMEDIATE transaction"
                    )
                    assert any("FROM campaigns" in s for s in sql), (
                        "launch brief write does not read the row in the same closure"
                    )
                    found = True
        assert found, "no nested closure in _launch_loop writes the brief"

    def test_activate_emergent_brief_write_is_inside_the_transaction(self):
        """Same pin for _activate_emergent: lock before the RMW read, brief
        rendered before commit."""
        src = inspect.getsource(h._activate_emergent)
        assert "BEGIN IMMEDIATE" in src
        # brief write must come BEFORE the commit in source order
        assert src.index("_write_brief") < src.index("db.commit()"), (
            "_activate_emergent renders the brief outside its write transaction"
        )
