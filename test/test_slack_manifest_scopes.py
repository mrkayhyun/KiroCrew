...
class TestManifestGrantsWhatTheCodeUses:
    ...
    def test_no_user_token_scopes(self) -> None:
        # The app runs on a bot token only. A user-token scope in the manifest
        # would widen the OAuth consent screen for surface no code path uses.
        scopes = _manifest()["oauth_config"]["scopes"]
        assert list(scopes.keys()) == ["bot"]
