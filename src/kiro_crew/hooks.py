...
def _expand_home_vars(value: str) -> str:
    ...
    NOT ``os.path.expandvars``, which dereferences ANY variable. That made the deny
    matcher an exfiltration primitive: a denied target is audited to the security
    event log, whose ``operation`` field is the target string, so an agent could pick
    a path prefix it knows a rule refuses, append ``$AWS_SECRET_ACCESS_KEY``, and have
    the deny it deliberately triggered write the expanded secret to a readable log.
    Expanding the home variables carries no such value — the home path is what a home
    rule matches on anyway — while still collapsing ``$HOME`` onto the one spelling.
...
def _normalize_search_path(value: str) -> str:
    ...
    ``os.path.expanduser`` resolves ``~name`` through ``pwd.getpwnam``, a synchronous
    NSS lookup that can stall the gateway event loop for seconds when the name comes
    from LDAP or another remote directory — and an agent controls this string, so
    ``path="~ldap-user/x"`` would make the permission gate itself the stall. A literal
    ``~name`` still matches rules by its literal spelling, the same contract as an
    unexpanded ``$VAR``.
