...
**Most browser commands run without asking the user.** Anything whose effect stays inside the page — open, click, type, snapshot, screenshot, tabs, cookies, storage, console — is auto-approved because the CLI being installed is itself the user's consent. Commands that reach the local machine still prompt, and that is deliberate, not a bug to route around: `eval` and `run-code` (arbitrary code in an authenticated page), `upload` (sends a local file to the page), `state-load` (reads an arbitrary local path), `state-save <name>` / `--filename` (arbitrary local write), and the installers. If you need one, run it and let the user approve; do not rewrite it into a form that dodges the prompt.
