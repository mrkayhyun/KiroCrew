...
from kiro_crew.browser_cli.install import cli_path
...
def stop() -> None:
    """Stop the dashboard and the CLI daemon behind it.

    ``show --kill`` is issued first: it stops the daemon the dashboard fronts,
    which reaping the supervised child alone does not do. It is best-effort —
    a failure there must not prevent the child from being reaped, or the port
    would stay held.
    """
    global _proc, _info
    with _lock:
        ...
        if cli is not None:
            try:
                subprocess.run(
                    [cli, "show", "--kill"],
                    capture_output=True,
                    timeout=_STOP_KILL_TIMEOUT_S,
                    check=False,
                )
            except (OSError, subprocess.SubprocessError) as exc:
                logger.debug("playwright-cli show --kill failed: %s", exc)
        if _proc is not None:
            _reap(_proc)
        _proc = None
        _info = None
