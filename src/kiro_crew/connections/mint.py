...
# Imported as a MODULE, not as symbols: the agents dir and the atomic writer are
# resolved at call time so a test (and the spec-emission path) can substitute
# them. ``from ... import f`` would freeze this module's own binding.
...
class MintState(TypedDict, total=False):
    ...
    reason: str
    started: float
    token: int  # row identity; see _mint_tokens
    client: Any
    watcher: Any
...
# Row identity. Deliberately NOT a clock reading: ``time.monotonic()`` has ~15.6ms
# granularity on Windows, so two Connects for one provider inside the same tick
# read as the same row and every token guard fails open -- the late-failure
# clobber those guards exist to prevent. A counter is unique by construction.
_mint_tokens = itertools.count(1)
...
def _acp_client_factory() -> Any:
    ...
    The import is deferred because the ACP client drags in the whole provider
    stack, which a mint only needs once a card actually asks for a URL.
    """
...
def _remove_mint_agent_spec(spec_path: str) -> None:
    ...
    except OSError:  # ephemeral file; the manifest self-heal catches leftovers
        logger.debug("mint agent spec removal failed", exc_info=True)
...
async def _mint_watcher(slug: str, mcp_url: str, token: int) -> None:
...
async def claim_expired_mint(slug: str, token: int) -> bool:
    """Consume ``slug``'s row iff it is still ``token``'s and still expired.

    The claim is what makes an entry removal safe to act on: it and the removal run
    in one critical section on the caller's side, so a superseding reserve either
    happens before the claim (which then fails) or after the removal (and installs
    its own row). Returns False for a superseded, missing or non-expired row.
    """
    async with _mints_lock:
        entry = _mints.get(slug)
        if entry is None or entry.get("token") != token or entry.get("state") != "expired":
            return False
        _mints.pop(slug, None)
        return True


async def reserve_mint_row(slug: str) -> tuple[int, MintState | None]:
...
async def start_oauth_mint(
    ...
    try:
        ...
        if isinstance(pid, int) and pid > 0:
            # Nothing claims this process as a session, so the orphan sweep would
            # reap it once it ages past the spawn grace -- well inside the TTL.
...
def pending_mint_for(slug: str) -> MintState | None:
    ...
    token = int(entry.get("token") or 0)
